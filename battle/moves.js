exports.BattleMovedex = {
	...
	"entrainment": {
		...
		onTryHit: function(target, source) {
			...
			var bannedTargetAbilities = {
				multitype: 1,
				stancechange: 1,
				truant: 1
			};
			var bannedSourceAbilities = {
				flowergift: 1,
				forecast: 1,
				illusion: 1,
				imposter: 1,
	...
	"fellstinger": {
		...
		effect: {
			...
			onAfterMoveSecondarySelf: function(pokemon, target, move) {
				if (!target || target.fainted || target.hp <= 0) this.boost({
					atk: 2
	...
	"firepledge": {
		...
		onPrepareHit: function(target, source, move) {
			for (var i = 0; i < this.queue.length; i++) {
				...
				if (decision.pokemon.side === source.side && decision.move.id in {
						grasspledge: 1,
						waterpledge: 1
					}) {
	...
	"flowershield": {
		...
		onHitField: function(target, source) {
			...
			for (var i = 0; i < targets.length; i++) this.boost({
				def: 1
			}, targets[i], source, this.getMove('Flower Shield'));
		},
		secondary: false,
		target: "all",
		type: "Fairy"
	...
	"foresight": {
		...
		effect: {
			...
			onNegateImmunity: function(pokemon, type) {
				if (pokemon.hasType('Ghost') && type in {
						'Normal': 1,
						'Fighting': 1
					}) return false;
			},
	...
	"growth": {
		...
		onModifyMove: function(move) {
			if (this.isWeather(['sunnyday', 'desolateland'])) move.boosts = {
				atk: 2,
				spa: 2
			};
		},
	...
	"magneticflux": {
		...
		onHitSide: function(side, source) {
			...
			for (var i = 0; i < targets.length; i++) this.boost({
				def: 1,
				spd: 1
			}, targets[i], source, 'move: Magnetic Flux');
		},
		secondary: false,
		target: "allySide",
		type: "Electric"
